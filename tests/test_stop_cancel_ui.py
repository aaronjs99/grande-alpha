...
@pytest.mark.asyncio
async def test_shadow_only_stop_never_reads_or_cancels_broker_orders(setup, monkeypatch):
    window, controller, broker, _, _, messages = setup
    controller.shadow_only_runtime = True

    async def forbidden():
        raise AssertionError("Shadow STOP must not inspect a live cancellation scope")

    monkeypatch.setattr(broker, "get_accounts", forbidden)
    await window._stop_and_cancel()
    assert messages[-1][0] == "Shadow stopped"
    assert broker.cancel_calls == []
