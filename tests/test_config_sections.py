...
def _legacy_flat(config: AppConfig) -> dict:
    grouped = config_document(config)
    return {"cadence_version": config.cadence_version, **{
        name: value for section in CONFIG_SECTIONS for name, value in grouped[section].items()
    }}
...
def test_every_setting_has_one_saved_section_and_round_trips(tmp_path):
    ...
    assert set(names) == set(_legacy_flat(AppConfig())) - {"cadence_version"}
    assert {item.name for item in fields(AppConfig)} == {"cadence_version", *CONFIG_SECTIONS}

    path = tmp_path / "config.json"
    config = AppConfig(broker=BrokerConfig(broker_connection_enabled=True, live_trading_enabled=True),
                       risk=RiskConfig(default_max_daily_loss=7.5))
...
def test_old_flat_configuration_needs_explicit_backed_up_upgrade(tmp_path):
    path = tmp_path / "config.json"
    original = _legacy_flat(AppConfig(broker=BrokerConfig(broker_connection_enabled=True,
...
def test_typed_sections_are_authoritative_without_flat_aliases():
    config = AppConfig()
    ...
    with pytest.raises(AttributeError):
        _ = config.default_max_daily_loss


def test_fresh_preferences_do_not_supply_live_financial_limits():
    risk = config_document(AppConfig())["risk"]
    assert risk and all(value is None for value in risk.values())
...
@pytest.mark.parametrize(
    ...
    [("broker", "live_trading_enabled", "yes"), ("risk", "default_max_daily_loss", -1),
     ("risk", "default_max_order_notional", float("nan")), ("risk", "default_max_trades", 1.5),
     ("data", "poll_seconds", True), ("strategy", "no_trade_open_minutes", -1)],
)
def test_invalid_settings_fail_before_save(tmp_path, section, setting, value):
    config = AppConfig()
