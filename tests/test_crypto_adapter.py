...
from grande_alpha.broker.base import BrokerError, ShadowOnlyBroker
...
@pytest.mark.asyncio
async def test_shadow_facade_cannot_acquire_crypto_mutation_tools():
    broker = ShadowOnlyBroker(object())
    with pytest.raises(BrokerError, match="blocks crypto"):
        await broker.review_crypto_order(ACCOUNT.account_number, INTENT)
    with pytest.raises(BrokerError, match="blocks crypto"):
        await broker.place_crypto_order(None)
    with pytest.raises(BrokerError, match="blocks crypto"):
        await broker.cancel_crypto_order(ACCOUNT.account_number, ORDER_ID)
