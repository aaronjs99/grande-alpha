...
def test_live_grant_requires_flat_order_free_and_fresh_exact_quotes(
    ...
    for name, positions, orders, quotes, message in cases:
        ...
        if name in {"missing-quote", "stale-quote"}:
            # Approval may happen outside the entry window; executable quote
            # freshness is checked when the strategy actually starts.
            controller.authorize_live(grant)
...
def test_premarket_approval_waits_for_entry_and_start_discards_prestart_pipeline(
    tmp_path, monkeypatch: pytest.MonkeyPatch
) -> None:
    controller, _broker, store, grant = _controller(
        tmp_path,
    ...
    assert controller.risk.grant is premarket_grant
    with pytest.raises(RuntimeError, match="outside the regular-session entry window"):
        controller.start_strategy()
...
def test_live_authority_restores_exact_candidate_entry_count_from_durable_fills(
    ...
    store.record_daily_risk(
        ACCOUNT_NUMBER,
        grant.starts_at.date().isoformat(),
        controller.snapshot.portfolio.total_value,
        grant.max_daily_loss,
    )
    controller.authorize_live(grant)
...
def test_live_readiness_tab_renders_operator_gates_and_next_actions(
    ...
    assert tab_index >= 0
    assert window.tabs.tabText(tab_index) == "Overview"
    assert window.tabs.overview.tabText(window.tabs.overview.indexOf(window.activation_widget)) == "Readiness checks"
    assert [
...
@pytest.mark.asyncio
async def test_authorize_and_start_prefers_evidence_route_with_exact_operator_label(
    tmp_path, monkeypatch: pytest.MonkeyPatch
) -> None:
    _qt_app()
    controller, _broker, store, _grant = _controller(
    ...
    async def fresh_read_only_truth() -> None:
        return None

    monkeypatch.setattr(controller, "safe_read_only_refresh", fresh_read_only_truth)
    monkeypatch.setattr(controller, "entry_window_open", lambda: True)
    monkeypatch.setattr(
        controller,
        "start_strategy",
    ...
    assert window.authorize_button.text() == "Review evidence-gated session"
    assert window.authorize_action.text().removesuffix("…") == "Review evidence-gated session"
    assert window.authorize_button.isEnabled()
    window.authorize_button.click()
    await window.tasks._tasks["authorize"]
...
async def test_live_window_keeps_pause_resume_revoke_and_stop_controls_visible(
    ...
    assert window.kill_button.text().lower().startswith("stop")
...
@pytest.mark.asyncio
async def test_window_exit_does_not_claim_broker_cleanup_was_verified(
    tmp_path, monkeypatch: pytest.MonkeyPatch
) -> None:
    app = _qt_app()
    controller, _broker, store, _grant = _controller(
    ...
    assert window._closing_after_cleanup
