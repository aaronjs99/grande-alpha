...
def test_pre_cadence_config_is_upgraded_to_retail_low_latency_defaults() -> None:
    ...
    assert upgraded["cadence_version"] == 6
    ...
    assert upgraded["market_history_retention_days"] == 240
...
def test_v5_default_retention_migrates_but_deliberate_custom_value_is_preserved() -> None:
    migrated = migrate_config_payload(
        {"cadence_version": 5, "market_history_retention_days": 90}
    )
    custom = migrate_config_payload(
        {"cadence_version": 5, "market_history_retention_days": 365}
    )

    assert migrated["cadence_version"] == 6
    assert migrated["market_history_retention_days"] == 240
    assert custom["market_history_retention_days"] == 365
