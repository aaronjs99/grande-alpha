...
For research-only earnings candidates, see [earnings screening](MIXED_PORTFOLIO.md).
...
terminal approval for every ticket; see [attended live CLI](#attended-live-cli).
...
Post-earnings-announcement drift (PEAD) now has a separate [research-only candidate screen](MIXED_PORTFOLIO.md).
...
Use `engine readiness` and the [autonomy handoff](LIVE_ACTIVATION.md) for consolidated next steps.
