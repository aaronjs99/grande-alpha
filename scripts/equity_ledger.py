...
class EquityLedger:
    ...
    def acquire_lease(self, account: str, owner: str, *, now: datetime, ttl_seconds: float = 30) -> None:
        ...
        with self._lock, self._db:
            ...
            if row is not None and row["owner"] != owner and datetime.fromisoformat(row["expires_at"]) > now.astimezone(UTC):
                raise RuntimeError("Another mixed engine holds the account execution lease")
            self._db.execute(
    ...
    def release_lease(self, account: str, owner: str) -> None:
        with self._lock, self._db:
            self._db.execute("DELETE FROM equity_v1_engine_lease WHERE account=? AND owner=?", (account, owner))
    ...
    def reserve(self, account: str, authority: str, intent: EquityOrderIntent):
        ...
        with self._lock, self._db:
            self._db.execute("INSERT INTO equity_v1_intents VALUES(?,?,?,?,?,NULL)",
                             (intent.ref_id, account, authority, json.dumps(intent.as_dict(), sort_keys=True), "reserved"))

    def mark_submitting(self, ref: str, *, submitted_at: datetime | None = None, notional: float | None = None):
        if (submitted_at is None) != (notional is None):
            raise ValueError("Dispatch time and notional must be supplied together")
        if submitted_at is not None:
            if (submitted_at.tzinfo is None or submitted_at.utcoffset() is None or isinstance(notional, bool)
                    or not isinstance(notional, (int, float)) or not math.isfinite(notional) or notional <= 0):
                raise ValueError("Invalid dispatch provenance")
        with self._lock, self._db:
            cursor = self._db.execute("UPDATE equity_v1_intents SET state='submitting' WHERE ref=? AND state='reserved'", (ref,))
            if cursor.rowcount != 1:
                raise ValueError("Intent already attempted or missing; do not resubmit")
            if submitted_at is not None:
                self._db.execute("INSERT INTO equity_v1_dispatch VALUES(?,?,?)",
    ...
    def daily_usage(self, account: str, day: str) -> dict:
        ...
        with self._lock:
            rows = self._db.execute("SELECT i.state,d.submitted_at,d.notional FROM equity_v1_intents i "
                                    "LEFT JOIN equity_v1_dispatch d ON i.ref=d.ref WHERE i.account=? "
                                    "AND i.state NOT IN ('reserved','abandoned')", (account,)).fetchall()
        count, amount = 0, 0.0
        for row in rows:
    ...
    def holding_open_times(self, account: str) -> dict[str, str]:
        with self._lock:
            rows = self._db.execute("SELECT symbol,side,quantity,executed_at FROM equity_v1_fills "
                                    "WHERE account=? ORDER BY executed_at,execution_id", (account,)).fetchall()
        quantities, opened = {}, {}
        for row in rows:
            symbol = row["symbol"]
    ...
    def mark_unknown(self, ref: str):
        with self._lock, self._db:
            cursor = self._db.execute("UPDATE equity_v1_intents SET state='unknown' WHERE ref=? AND state='submitting'", (ref,))
            if cursor.rowcount != 1:
    ...
    def abandon_reserved(self, ref: str):
        ...
        with self._lock, self._db:
            cursor = self._db.execute("UPDATE equity_v1_intents SET state='abandoned' WHERE ref=? AND state='reserved'", (ref,))
            if cursor.rowcount != 1:
                raise ValueError("Attempted orders cannot be abandoned or reset")

    def unresolved(self, account: str) -> list[dict]:
        with self._lock:
            rows = self._db.execute("SELECT ref,state,order_id FROM equity_v1_intents WHERE account=? "
                                    "AND state IN ('reserved','submitting','unknown','open')", (account,)).fetchall()
    ...
    def observe(self, account: str, ref: str, order: BrokerOrder, *, observed_at: datetime):
        ...
        with self._lock, self._db:
            row = self._db.execute("SELECT * FROM equity_v1_intents WHERE ref=? AND account=?", (ref, account)).fetchone()
            ...
            if order.created_at is None or order.created_at < datetime.fromisoformat(intent["created_at"])-timedelta(seconds=5):
                raise ValueError("Provider order predates the recorded ticket")
            if row["order_id"] is None and order.raw.get("ref_id") != ref:
                raise ValueError("Unbound order requires exact provider reference; similarity is not identity")
            if row["order_id"] is not None and row["order_id"] != order.order_id:
                raise ValueError("Provider order identity changed")
            if order.raw.get("ref_id", ref) != ref or (order.symbol, order.side) != (intent["symbol"], intent["side"]):
                raise ValueError("Provider order does not match the recorded ticket")
            for raw_key, intent_key in (("type", "order_type"), ("time_in_force", "time_in_force"), ("market_hours", "market_hours")):
            ...
            existing = self._db.execute("SELECT execution_id FROM equity_v1_fills WHERE ref=?", (ref,)).fetchall()
            ...
            for execution in order.executions:
                values = (account, execution.execution_id, ref, order.symbol, order.side, execution.quantity,
                          execution.price, execution.fees, execution.timestamp.astimezone(UTC).isoformat())
                previous = self._db.execute("SELECT * FROM equity_v1_fills WHERE account=? AND execution_id=?",
                                            (account, execution.execution_id)).fetchone()
            ...
            self._db.execute("UPDATE equity_v1_intents SET order_id=?,state=? WHERE ref=?",
                             (order.order_id, "terminal" if terminal else "open", ref))

    def inventory(self, account: str) -> dict[str, float]:
        with self._lock:
            rows = self._db.execute("SELECT symbol,side,quantity FROM equity_v1_fills WHERE account=? "
                                    "ORDER BY executed_at,execution_id", (account,)).fetchall()
        totals = {}
        for row in rows:
            symbol = row["symbol"]
