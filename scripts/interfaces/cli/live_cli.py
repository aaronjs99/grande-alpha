...
from grande_alpha.domain.models import LiveGrant, OrderConfirmationDecision, OrderConfirmationRequest, utc_now
...
def _session_policy(args: Any) -> tuple[str, LiveGrant, dict | None]:
    ...
    terms = None
    if unattended:
        from grande_alpha.execution.standing import validate_terms
...
def command_live(args: Any) -> int:
    ...
    from grande_alpha.configuration.config import data_dir, load_config
    ...
    try:
        ...
        store = AuditStore()
        from grande_alpha.desktop.device_notifications import display
...
def command_standing_template(args: Any) -> int:
    from grande_alpha.execution.standing import STANDING_TERMS
