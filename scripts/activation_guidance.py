...
def external_guidance_links(raw: str | None = None) -> tuple[ExternalGuidanceLink, ...]:
    """Return product links and validated distributor-configured resources."""
    ...
    if not isinstance(values, list):
        return DEFAULT_EXTERNAL_GUIDANCE_LINKS
    custom = tuple(link for link in (_validated_link(value) for value in values[:8]) if link is not None)
