...
at 6:20 AM local time on weekdays. Local installation adds only the setup shortcut; it never enables
the task silently.
...
Version `0.14.0` makes the evidence-backed CASH / hold policy the normal and scheduled runtime champion. A purged three-fold development tournament at 3x modeled costs found every intraday family negative; the terminal five-session block remains untouched. Shadow decisions now fill on the first causal next-bar quote instead of an extra bar later. Evidence policy v9 binds every material execution and sizing field, applies true all-component cost stress, rejects failure-bypassing forced closes, and makes the remaining replay/runtime sizing mismatch a visible promotion blocker. No strategy has live authority, so scheduled operation remains engineering-only read-only shadow. See the [champion selection report](docs/CHAMPION_SELECTION_2026-08-11.md), [scheduled shadow](docs/SCHEDULED_SHADOW.md), [trading sessions](docs/TRADING_SESSIONS.md), [Action Lab methodology](docs/ACTION_LAB.md), and the [public release checklist](docs/PUBLIC_RELEASE_CHECKLIST.md).
