...
- **STOP + CANCEL** first locks new local requests and shows an exact confirmation of the nonterminal Agentic-account orders owned by GRANDE Alpha's durable intent ledger. It sends cancellation requests only after explicit confirmation, never includes unrelated or manually placed orders, and discloses already-pending cancellations for terminal verification without submitting them twice. A connection with no recorded GRANDE order activity stops without another broker read. Disconnect and Exit can be explicitly confirmed without verified cleanup; they retain order records and never silently cancel. A confirmed cancellation can still fail during a network or provider failure and never liquidates a filled position.
...
Equities and Crypto run as concurrent research workers. The Agent desk shows six animated
bot cards, a Pacific-time balance chart and activity log, source status, and a persistent
**Talk to your AI team** panel (an **AI chat** tab on smaller windows). Chat with your
installed Ollama model and review paper-control suggestions before applying them.
**Prompts + AI connections** provides editable team/worker prompts and a default-off local
MCP connection for compatible AI clients. **ChatGPT Astra setup** guides you through adding the desktop connection, allowing
...
Use the **Dark mode / Light mode** button in the top bar, or **View → Dark mode**, to change
the appearance of the whole app. The selection is remembered across restarts and is separate
from trading configuration.

**STOP + CANCEL** stops the Agent, trading strategy, and live shadow and revokes live
authority before checking Robinhood. A status message reports progress, no eligible orders,
or any failure. If GRANDE-owned ETF orders are open, review and confirm the exact cancellation
list; the app then checks their terminal state. Filled positions and unrelated orders remain
untouched. Robinhood stays connected so you can inspect account status; use **Disconnect**
to end the connection. A slow order preview times out after 30 seconds with automation stopped.
See the [stop status example](docs/images/stop-cancel-status.png).

Disconnect and Exit pause polling and stop local automation. With no recorded GRANDE order
activity they close the transport directly. If prior orders exist, cleanup reads are bounded;
when cleanup cannot be verified, a default-No dialog lets you deliberately leave and manage
orders in Robinhood. Order records remain available for reconciliation on the next connection.
See the [disconnected status example](docs/images/disconnect-status.png).
